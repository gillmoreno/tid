...
import { tickArticleScheduler } from "@/api/articles";
...
interface ArticleScheduledQueueProps {
  posts: ArticleCandidate[];
  onTick: () => void;
}

export function ArticleScheduledQueue({ posts, onTick }: ArticleScheduledQueueProps) {
  ...
  async function handleTick() {
    ...
    setResult(null);
    try {
      const res = await tickArticleScheduler();
      onTick();
      if (res.prepared.length === 0) {
        setResult("No posts due right now.");
      } else {
        setResult(`Prepared ${res.prepared.length} post(s) — clipboard + Chrome.`);
      }
    } catch {
  ...
  return (
    <div className="rounded-lg border border-line bg-ink-soft">
      ...
      {result && <p className="border-b border-line px-5 py-2 font-mono text-[10px] text-fog">{result}</p>}
            ...
            <li key={post.id} className="px-5 py-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-sm text-white">
                  {post.scheduled_at ? formatScheduledAt(post.scheduled_at) : "—"}
                </span>
                <span className={factoryStatusBadge(post.status)}>{post.status}</span>
              </div>
              <p className="mt-1 truncate text-xs text-fog">{post.post_text.slice(0, 80) || post.id}</p>
              {post.prepared_at && (
